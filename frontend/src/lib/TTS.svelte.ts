...
class AudioQueue {
    ...
    // On change 'add' pour qu'il empile les promesses
    add = (audioId: string) => {
    ...
    private async streamAudio(audioId: string) {
        ...
        try {
            ...
            console.log(`%c📥 [Fetch] Début du traitement séquentiel pour : ${audioId}`, "color: #fbbf24");
            ...
            while (true) {
                ...
                if (!chunk || chunk.length === 0) { if (done) break; continue; }
                if (chunk.length % 2 !== 0) {
                ...
                console.log(`%c🎵 [Play] ${audioId.split('-')[0]}... programmé à ${startTime.toFixed(2)}s`, "color: #4ade80");
