...
import { sendMessage as sendWsMessage, chatState } from '$lib/websocket.svelte';
...
export function createChatStore() {
	...
	async function ChargerConversation(id: number) {
		...
		const idx = historiques.findIndex((h) => h.id === id);
		if (idx > 0) {
			const [item] = historiques.splice(idx, 1);
			historiques = [item, ...historiques];
		}
		...
		if (res.ok) {
			...
			messages = data
				.map((msg: { role: string; content: string; image?: string | null }) => {
	...
	function syncWithWebSocket() {
		if (chatState.isThinking && messages[0] && messages[0].role === 'assistant') {
			...
			messages[0].status =
				chatState.contentStream.length > 0
